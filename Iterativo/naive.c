    ...
    *ingredient_size = 1;
    ...
    for (i = 0; i < cuisine_size; i++){
        for (j = 0; j < cuisine_list[i].distintic_ingredients; j++){
            ...
            //not new
            if (aux < *ingredient_size){
                global[aux].amount = global[aux].amount + cuisine_list[i].ingredient_list[j].amount;
            }else{
                //new
                global = (ingredient_t *) realloc (global, (aux + 1) * sizeof(ingredient_t));
                strcpy(global[aux].name, cuisine_list[i].ingredient_list[j].name);
                global[aux].amount = cuisine_list[i].ingredient_list[j].amount;
...
int findFrequency(cuisine_t *cuisines, int size_cuisines, char *cuisineName, char *ingredient){
    ...
    if (cozinha < size_cuisines){

        for (j = 0; j < cuisines[cozinha].distintic_ingredients; j++){
            if (strcmp(cuisines[cozinha].ingredient_list[j].name, ingredient) == 0){
                ret = cuisines[cozinha].ingredient_list[j].amount;
            }
        }
